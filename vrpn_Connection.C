// This DEF should be just in the header, if you want to use it. I leave it in both, so the editor can notice
//#define EVAN_CODE
#ifdef EVAN_CODE  
...
, d_anonList(NULL)
{
	FILE* portFile = fopen("vrpn_ports.txt", "r");

	if (portFile == NULL)
	{
		outgoingPortList = nullptr;
		numOutgoingPorts = 0;
	}
	else
	{
		outgoingPortList = new vrpn_uint32[65535];
		unsigned int port = 0;
		while (fscanf(portFile, "%d", &port) != EOF)
		{
			outgoingPortList[numOutgoingPorts] = port;
			numOutgoingPorts++;
		}
		fclose(portFile);
		...
		srand(time(NULL));
		if (numOutgoingPorts > 1)
		{
			for (size_t i = 0; i < numOutgoingPorts; i++)
			{
				size_t j = rand() % numOutgoingPorts;
				vrpn_uint32 temp = outgoingPortList[j];
				outgoingPortList[j] = outgoingPortList[i];
				outgoingPortList[i] = temp;
			}
		}
	}
}
...
// added by Evan A. Suma
vrpn_uint32* vrpn_ConnectionManager::getOutgoingPortList()
{
	return outgoingPortList;
}

// added by Evan A. Suma
vrpn_uint32 vrpn_ConnectionManager::getNumOutgoingPorts()
{
	return numOutgoingPorts;
}
	...
	const char *IPaddress)
{
	...
	// modified by Evan A. Suma
	vrpn_uint32* outgoingPortList = vrpn_ConnectionManager::instance().getOutgoingPortList();
	vrpn_uint32 numOutgoingPorts = vrpn_ConnectionManager::instance().getNumOutgoingPorts();

	if (ntohs(name.sin_port) == 0 && numOutgoingPorts != 0 && outgoingPortList != nullptr)
	{
		int done = -1;
		for (vrpn_uint32 i = 0; i < numOutgoingPorts && done == -1; i++)
		{
			name.sin_port = htons(outgoingPortList[i]);
			done = bind(sock, (struct sockaddr *)&name, namelen) == 0;
		}

		if (done==-1)
		{
			fprintf(stderr, "open_socket:  can't bind address");
			fprintf(stderr, "  (This probably means that another application has taken all available ports)\n");
			vrpn_closeSocket(sock);
			return INVALID_SOCKET;
		}
	}
	...
	const char *NIC_IP = NULL)
{
	...
	unsigned short udp_port = static_cast<unsigned short>(INADDR_ANY);
	udp_socket = open_udp_socket(&udp_port, NIC_IP);
	...
	const char *NIC_IP = NULL)
{
	...
	* remote server. */
	...
	// modified by Evan A. Suma
	unsigned short tcp_port = static_cast<unsigned short>(INADDR_ANY);
	*listen_sock = open_tcp_socket(&tcp_port, NIC_IP);
	if (*listen_sock < 0) {
...
int vrpn_Endpoint_IP::connect_tcp_to(const char *addr, int port)
{
	...
	/* set up the socket */
	unsigned short tcp_port = static_cast<unsigned short>(INADDR_ANY);
	d_tcpSocket = open_tcp_socket(&tcp_port, d_NICaddress);
	if (d_tcpSocket < 0) {
